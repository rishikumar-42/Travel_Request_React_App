...
function EditTravelRequestForm() {
  ...
  const carRentalCategorySwitch =
    item.carRentalCategory !== "" && item.carRentalCategory != null;
  const HotelSwitch = item.hotelLocation !== "" && item.hotelLocation != null;
  const personCarSwitch =
    item.personalCarDrivingLicenseNumber !== "" &&
    item.personalCarDrivingLicenseNumber != null;
  ...
  const [isTelephoneNumberValid, setIsTelephoneNumberValid] = useState(
    /^\d{9,15}$/.test(item.issuerNumber)
  );
  ...
  const [employeeDropDownSuggestions, setEmployeeDropDownSuggestions] =
    useState([]);
  ...
  const handleRemovefiles = async (rowIndex) => {
    ...
    try {
      ...
      showMessage(
        "success",
        "Success",
        `Successfully removed ${selectedFile.title}`
      );
  ...
  const onFileUpload = async (selectedFile, originalFileName) => {
    ...
    try {
      const fileResponse = await TravelRequestFormService.addDocuments(
        selectedFile
      );
      const tempFile = {
      ...
      showMessage(
        "success",
        "Success",
        `Successfully uploaded ${fileResponse.title}`
      );
  ...
  const OnwardJourneyLink = (rowData) => {
    ...
    let urlObj = new URL(
      rowData.contentUrl,
      `${process.env.REACT_APP_API_LIFERAY_BASE_URL}`
    );
  ...
  const handleEditItinerary = (index) => {
    ...
    setNewItinerary({
      ...itineraryToEdit,
      onwardDepartureDate: itineraryToEdit.onwardDepartureDate
        ? new Date(itineraryToEdit.onwardDepartureDate)
        : null,
      returnArrivalDate: itineraryToEdit.returnArrivalDate
        ? new Date(itineraryToEdit.returnArrivalDate)
        : null,
    });
  ...
  const handleSaveItinerary = () => {
    if (editingItinerary !== null) {
      ...
      setItineraries(
        itineraries.map((it, i) => (i === editingItinerary ? newItinerary : it))
      );
  ...
  const deleteItineraryFromServer = async (id) => {
    try {
      await fetch(
        `${process.env.REACT_APP_API_LIFERAY_BASE_URL}/o/c/traveldetailses/${id}`,
        {
          method: "DELETE",
          headers: {
  ...
  const searchItem2 = (event) => {
    ...
    setdropDownSuggestions2(
      userList.filter(
        (item) =>
          item.email.toLowerCase().includes(query) &&
          item.email.toLowerCase() !== formData.manager.toLowerCase() &&
          item.email.toLowerCase() !== formData.email.toLowerCase()
      )
    );
  };
  ...
  useEffect(() => {
    const getAllCurrency = async () => {
      try {
        const currencyList =
          await TravelRequestFormService.fetchCurrencyPicklist();
    ...
    const getAllPreferredTime = async () => {
      try {
        const preferredTimeList =
          await TravelRequestFormService.fetchPreferredTimePicklist();
  ...
  useEffect(() => {
    const getAllFlightTypes = async () => {
      ...
      try {
        const flightTypes =
          await TravelRequestFormService.fetchFlightTypePicklist();
  ...
  useEffect(() => {
    const getAllTrainTypes = async () => {
      try {
        const trainTypes =
          await TravelRequestFormService.fetchTrainTicketTypePicklist();
  ...
  const [formData, setFormData] = React.useState({
    ...
    issuer: item.issuer || "",
    issuerDate: item.issuerDate ? new Date(item.issuerDate) : null,
  ...
  const validateForm = () => {
    ...
    if (!formData.travelDepartureDate)
      newErrors.travelDepartureDate = "Departure date is required.";
    if (!formData.travelArrivalDate)
      newErrors.travelArrivalDate = "Arrival date is required.";
    if (formData.travelCurrency.key === undefined)
      newErrors.travelCurrency = "Travel Currency is required.";
    ...
    if (
      (formData.travelType === "international" ||
        formData.travelType === "both") &&
      !formData.hod
    )
      newErrors.hod = "Email is required.";

    if (showNights) {
      if (!formData.hotelLocation)
        newErrors.hotelLocation = "Hotel location is required.";
      if (!formData.hotelCheckIn)
        newErrors.hotelCheckIn = "Hotel CheckIn is required.";
      if (!formData.hotelCheckOut)
        newErrors.hotelCheckOut = "Hotel CheckOut is required.";
    }

    if (showCarDetails) {
      if (!formData.carRentalFrom)
        newErrors.carRentalFrom = "Car rental from is required.";
      if (!formData.carRentalTo)
        newErrors.carRentalTo = "Car rental to is required.";
      if (!formData.carRentalOn)
        newErrors.carRentalOn = "Car rental on is required.";
      if (!formData.carRentalUntil)
        newErrors.carRentalUntil = "Car rental until is required.";
      if (!formData.carDrivingLicense)
        newErrors.carDrivingLicense = "Car rental driving License is required.";
      if (!formData.carRentalCategory)
        newErrors.carRentalCategory = "Car rental category is required.";
    }

    if (showPerCarDetails) {
      if (!formData.personalCarDrivingLicenseNumber)
        newErrors.personalCarDrivingLicenseNumber =
          "Perasonal Car driving License is required.";
      if (!formData.personalCarRegistrationNumber)
        newErrors.personalCarRegistrationNumber =
          "Personal Car registration no. is required.";
    }

    if (showFlightTicket) {
      if (!formData.flightTicketType)
        newErrors.flightTicketType = "Flight Ticket Type is required.";
    }
  ...
  useEffect(() => {
    ...
    if (
      newItinerary.onwardJourneyFrom !== "" &&
      newItinerary.onwardJourneyTo !== "" &&
      newItinerary.onwardDepartureDate !== null &&
      newItinerary.onwardTime !== null &&
      newItinerary.onwardTransportNumber !== "" &&
      newItinerary.budget > 0 &&
      !showReturnFields
    ) {
      ...
      newItinerary.budget > 0 &&
      showReturnFields &&
      newItinerary.returnJourneyFrom !== "" &&
      newItinerary.returnJourneyTo !== "" &&
      newItinerary.returnArrivalDate !== null &&
      newItinerary.returnTime !== null &&
      newItinerary.returnTransportNumber !== ""
    ) {
      setSaveItineraryFlag(false);
    } else {
  ...
  const handleBlur = (value) => {
    if (value === "") setIsEmployeeEmailValid(false);
    else setIsEmployeeEmailValid(validateEmployeeEmail(value));
  };
  ...
  const handleBlur2 = (value) => {
    if (value === "") setIsManagerEmailValid(false);
    else setIsManagerEmailValid(validateManagerEmail(value));
  };
  ...
  const handleBlur3 = (value) => {
    if (selectedItem === "") setIsHODEmailValid(true);
    else setIsHODEmailValid(validateHODEmail(value));
  };
  // const action = (
  //     <React.Fragment>
      ...
      <div className="bg-white align-items-start px-3 rounded-bottom-2 mb-3 pt-3 pb-3 shadow-sm">
        ...
        >
          ...
          <div className="d-flex justify-content-between gap-3 px-3 pt-3">
            <div className="p-inputgroup d-block">
              <FloatLabel>
                ...
                />
                <label htmlFor="issuer" className="small">
                  Issuer
                </label>
              </FloatLabel>
            ...
            <div className="p-inputgroup d-block">
              <FloatLabel>
                <Calendar
                  ...
                  showIcon
                />
                <label htmlFor="issuerDate" className="small">
                  Issue Date
                </label>
              </FloatLabel>
            ...
            <div className="p-inputgroup d-block">
              <FloatLabel>
                ...
                />
                <label htmlFor="number-input" className="small">
                  Telephone Number
                </label>
              </FloatLabel>
              {/* {(!isTelephoneNumberValid && formData.issuerNumber !== null) && <span htmlFor="number-input" style={{ color: 'red' }}>Number must be between 9 and 15 digits</span>}
                ...
                <div className="form-single">
                  <FloatLabel className="w-100">
                    <AutoComplete
                      ...
                      required
                    />
                    <label htmlFor="employeeEmail" className="small">
                      <strong>
                        Email <span className="text-danger px-1">*</span>
                      </strong>
                    </label>
                    {!isEmployeeEmailValid && (
                      <span htmlFor="employeeEmail" style={{ color: "red" }}>
                        Invalid Email
                      </span>
                    )}
                    {errors.email &&
                ...
                <div className="form-single">
                  <FloatLabel>
                    ...
                    <label htmlFor="name" className="small">
                      Name<span className="text-danger px-1">*</span>
                    </label>
                  </FloatLabel>
                </div>
                {/* <div className="form-single">
                ...
                <div className="form-single">
                  <FloatLabel>
                    <InputText
                      ...
                      readOnly
                    />
                    <label htmlFor="employeeNumber" className="small">
                      Employee Number<span className="text-danger px-1">*</span>
                    </label>
                  </FloatLabel>
                </div>
              </div>
            ...
            <div className="form-single-special">
              <FloatLabel>
                ...
                <label htmlFor="costCenter" className="small">
                  Cost Centre<span className="text-danger px-1">*</span>
                </label>
              </FloatLabel>
            </div>
            <div className="form-single-special">
              <FloatLabel>
                <InputText
                  ...
                  readOnly
                />
                <label htmlFor="entity" className="small">
                  Entity<span className="text-danger px-1">*</span>
                </label>
              </FloatLabel>
            </div>
            <div className="form-single-special">
              <FloatLabel>
                ...
                />
                <label htmlFor="positionTitle" className="small">
                  Position Title
                </label>
              </FloatLabel>
            </div>
          ...
          <div className="travel-details-container gap-3 px-3 pt-3">
            ...
            <p className="my-2">
              Note: All International flights have to be approved by Head of
              Department/GM/VP.
            </p>
            <div className="travel-type mb-3">
              <label htmlFor="travelType" className="mr-2">
                Travel Type<span className="text-danger px-1">*</span>
              </label>
              <div className="radio-group">
                <RadioButton
                  ...
                  disabled={item.approveStatus?.key !== "draft"}
                />
                <label htmlFor="domestic" className="mr-1 small">
                  Domestic
                </label>
                <RadioButton
                  ...
                  disabled={item.approveStatus?.key !== "draft"}
                />
                <label htmlFor="international" className="mr-1 small">
                  International
                </label>
                <label htmlFor="domestic" className="mr-1 small">
                  Domestic
                ...
                <RadioButton
                  ...
                  disabled={item.approveStatus?.key !== "draft"}
                />
                <label htmlFor="both" className="mr-1 small">
                  Both
                </label>
              </div>
            </div>
            {errors.travelType && !formData.travelType && (
              <span style={{ color: "red" }}>{errors.travelType}</span>
            )}
            <div className="d-flex justify-content-between align-items-stretch gap-3 my-4">
              <FloatLabel className="w-50">
                <InputTextarea
                  ...
                  cols={30}
                />
                <label htmlFor="travelPurpose" className="small">
                  Travel Purpose<span className="text-danger px-1">*</span>
                </label>
                {errors.travelPurpose && !formData.travelPurpose && (
                  <span style={{ color: "red" }}>{errors.travelPurpose}</span>
                )}
              </FloatLabel>
              <FloatLabel className="w-50">
                <InputTextarea
                  ...
                  cols={30}
                />
                <label htmlFor="participants" className="small">
                  Participants
                </label>
              </FloatLabel>
            </div>
            <div className="d-flex justify-content-between align-items-stretch gap-3 my-4">
              <div className="w-25">
                <FloatLabel>
                  <InputText
                    id="destination"
                    className="full-width-textarea w-100"
                    ...
                    cols={30}
                  />
                  <label htmlFor="destination" className="small">
                    Destination<span className="text-danger px-1">*</span>
                  </label>
                </FloatLabel>
              ...
              <div className="w-25">
                <FloatLabel>
                  <Calendar
                    ...
                    showIcon
                  />
                  <label htmlFor="departureDate" className="mr-2 small">
                    Departure Date<span className="text-danger px-1">*</span>
                  </label>
                </FloatLabel>
                {errors.travelDepartureDate &&
                  !formData.travelDepartureDate && (
              ...
              <div className="w-25">
                <FloatLabel>
                  <Calendar
                    id="returnDate"
                    minDate={formData.travelDepartureDate}
                    dateFormat="dd-M-yy"
                    className="w-100"
                    ...
                    showIcon
                  />
                  <label htmlFor="returnDate" className="mr-2 small">
                    Return Date<span className="text-danger px-1">*</span>
                  </label>
                </FloatLabel>
                {errors.travelArrivalDate && !formData.travelArrivalDate && (
                  <span style={{ color: "red" }}>
                    {errors.travelArrivalDate}
                  </span>
                )}
              </div>
              <div className="w-25">
                <FloatLabel>
                  <label htmlFor="estimatedduration" className="mr-2 small">
                    Estimated Duration
                  ...
                  <InputNumber
                    ...
                    className="w-100"
                    value={formData.travelEstimatedDuration}
                    readOnly
                  />
                </FloatLabel>
              </div>
              ...
              <div>
                <FloatLabel>
                  <Dropdown
                    ...
                    optionValue="name"
                  />
                  <label htmlFor="currency" className="small">
                    Currency<span className="text-danger px-1">*</span>
                  </label>
                </FloatLabel>
                {errors.travelCurrency &&
                  formData.travelCurrency.key === undefined && (
                    <span style={{ color: "red" }}>
                      {errors.travelCurrency}
                    </span>
                  )}
              ...
              <div className="w-50">
                <FloatLabel className="w-100">
                  <AutoComplete
                    ...
                    // tooltip="Disabled"
                  />
                  <label htmlFor="manager" className="small">
                    Manager<span className="text-danger px-1">*</span>
                  </label>
                  {!isManagerEmailValid && (
                    <span htmlFor="manager" style={{ color: "red" }}>
                      Invalid Email
                    </span>
                  )}
                  {errors.manager &&
              ...
              <div className="w-50">
                <FloatLabel className="w-100">
                  ...
                  <AutoComplete
                    ...
                    // tooltip="Disabled"
                  />
                  <label htmlFor="hod" className="small">
                    Head Of Department/GM/VP
                    {(formData.travelType === "international" ||
                      formData.travelType === "both") && (
                      <span className="text-danger px-1">*</span>
                    )}
                  ...
                  {!isHODEmailValid && (
                    <span htmlFor="hod" style={{ color: "red" }}>
                      Invalid Email
                    </span>
                  )}
                  {errors.hod && !formData.hod && isHODEmailValid && (
                    <span style={{ color: "red" }}>{errors.hod}</span>
                  )}
                </FloatLabel>
              </div>
            </div>
          ...
          <div className="hotel-container">
            ...
            <div className="form-row-toggle mx-2">
              <label className="toggle-label" htmlFor="hotelToggle">
                Hotel
              </label>
            ...
            {showNights && (
              ...
              <div className="d-flex align-items-stretch gap-3 my-4 mx-2">
                <div className="calendar-item">
                  ...
                  {errors.hotelLocation && !formData.hotelLocation && (
                    <span style={{ color: "red" }}>{errors.hotelLocation}</span>
                  )}
                </div>
                <div className="calendar-item col-width">
                  {/* <FloatLabel> */}
                  <lable className="startdate">
                    Check In<span className="text-danger px-1">*</span>
                  </lable>
                  <DatePicker
                    ...
                    shouldDisableDate={(date) => {
                      const travelArrivalDate = formData.travelArrivalDate
                        ? new Date(formData.travelArrivalDate).setHours(
                            0,
                            0,
                            0,
                            0
                        ...
                        : null;
                      const travelDepartureDate = formData.travelDepartureDate
                        ? new Date(formData.travelDepartureDate).setHours(
                            0,
                            0,
                            0,
                            0
                      ...
                      return (
                        date > travelArrivalDate || date < travelDepartureDate
                      ); // Disable dates before check-in
                    }}
                  />
                  {errors.hotelCheckIn && !formData.hotelCheckIn && (
                    <span style={{ color: "red" }}>{errors.hotelCheckIn}</span>
                  )}
                </div>
                <div className="calendar-item col-width">
                  {/* <FloatLabel> */}
                  <lable className="endate">
                    Check Out<span className="text-danger px-1">*</span>
                  </lable>
                  <DatePicker
                    format="dd-MMM-yyyy hh:mm aa"
                    showMeridian
                    id="checkOut"
                    minDate={formData.hotelCheckIn}
                    ...
                    // }}
                    shouldDisableDate={(date) => {
                      const checkInDate = formData.hotelCheckIn
                        ? new Date(formData.hotelCheckIn)
                        : formData.travelDepartureDate
                        ? new Date(formData.travelDepartureDate)
                        : null;
                      const travelArrivalDate = formData.travelArrivalDate
                        ? new Date(formData.travelArrivalDate)
                        : null;
                      return date <= checkInDate || date > travelArrivalDate; // Disable dates before check-in
                    }}
                  />
                  {errors.hotelCheckOut && !formData.hotelCheckOut && (
                    <span style={{ color: "red" }}>{errors.hotelCheckOut}</span>
                  )}
                </div>
                <div className="calendar-item">
                  <FloatLabel>
          ...
          <div className="car-container">
            <hr className="separator mb-2" />
            <div className="form-row-toggle mx-2">
              <label className="toggle-label" htmlFor="carRentalToggle">
                Car Rental
              </label>
              <label className="switch">
                <input
              ...
              <div className="">
                <div className="calendar-container d-flex justify-content-between gap-3 my-4 mx-2">
                  <div className="calendar-item">
                    ...
                    {errors.carRentalCategory &&
                      !formData.carRentalCategory && (
                        <span style={{ color: "red" }}>
                          {errors.carRentalCategory}
                        </span>
                      )}
                  ...
                  <div className="calendar-item">
                    <FloatLabel>
                      <label htmlFor="from">
                        From:<span className="text-danger px-1">*</span>
                      </label>
                      <InputText
                        type="text"
                        id="from"
                    ...
                    {errors.carRentalFrom && !formData.carRentalFrom && (
                      <span style={{ color: "red" }}>
                        {errors.carRentalFrom}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="calendar-item">
                    <FloatLabel>
                      <label htmlFor="to">
                        To:<span className="text-danger px-1">*</span>
                      </label>
                      <InputText
                        type="text"
                        id="to"
                    ...
                    {errors.carRentalTo && !formData.carRentalTo && (
                      <span style={{ color: "red" }}>{errors.carRentalTo}</span>
                    )}
                  </div>
                  <div className="calendar-item">
                    <FloatLabel>
                      <Calendar
                        id="on"
                        minDate={formData.travelDepartureDate}
                        maxDate={formData.travelArrivalDate}
                        dateFormat="dd-M-yy"
                      ...
                      />
                      <label for="on">
                        On:<span className="text-danger px-1">*</span>
                      </label>
                    </FloatLabel>
                    {errors.carRentalOn && !formData.carRentalOn && (
                      <span style={{ color: "red" }}>{errors.carRentalOn}</span>
                    )}
                  </div>
                  <div className="calendar-item">
                    <FloatLabel>
                      ...
                      />
                      <label for="until">
                        Until:<span className="text-danger px-1">*</span>
                      </label>
                    </FloatLabel>
                    {errors.carRentalUntil && !formData.carRentalUntil && (
                      <span style={{ color: "red" }}>
                        {errors.carRentalUntil}
                      </span>
                    )}
                  </div>
                </div>
                <div className="calendar-container d-flex align-items-stretch gap-3 my-4 mx-2">
                  <div className="calendar-item">
                    ...
                    {errors.carDrivingLicense &&
                      !formData.carDrivingLicense && (
                        <span style={{ color: "red" }}>
                          {errors.carDrivingLicense}
                        </span>
                      )}
          ...
          <div className="personalcar-container">
            <hr className="separator mb-2" />
            <div className="form-row-toggle mx-2">
              <label className="toggle-label" htmlFor="perCarToggle">
                Personal Car
              </label>
              <label className="switch">
                <input
            ...
            {showPerCarDetails && (
              <div className="calendar-container d-flex align-items-stretch gap-3 my-4 mx-2">
                <div className="calendar-item">
                  ...
                  {errors.personalCarRegistrationNumber &&
                    !formData.personalCarRegistrationNumber && (
                      <span style={{ color: "red" }}>
                        {errors.personalCarRegistrationNumber}
                      </span>
                    )}
                ...
                <div className="calendar-item">
                  <FloatLabel>
                    <label htmlFor="drivingLicenseNum">
                  ...
                  {errors.personalCarDrivingLicenseNumber &&
                    !formData.personalCarDrivingLicenseNumber && (
                      <span style={{ color: "red" }}>
                        {errors.personalCarDrivingLicenseNumber}
                      </span>
                    )}
          ...
          <div className="flightticket-container">
            <hr className="separator mb-2" />
            <div className="form-row-toggle mx-2">
              <label className="toggle-label" htmlFor="flightToggle">
                Travel Itinerary
              </label>
              <label className="switch">
                <input
              ...
              <div>
                <div className="d-flex flight-ticket-container mb-3">
                  <div className="d-flex gap-3 mt-3 mx-2">
                    <label htmlFor="travelType" className="mr-2">
                      Ticket Type<span className="text-danger px-1">*</span>
                    </label>
                    {flightTypeList.map((category) => {
                      return (
                        ...
                        >
                          ...
                          />
                          <label htmlFor={category.key} className="ps-1 px-2">
                            {category.name}
                          </label>
                        </div>
                      );
                  ...
                  <div className="d-flex gap-3 mt-3 mx-2 train">
                    <label htmlFor="travelType" className="mr-2 train">
                      Ticket Type<span className="text-danger px-1">*</span>
                    </label>
                      ...
                      return (
                        ...
                        >
                          <RadioButton
                            ...
                            checked={
                              trainTypeValue
                                ? trainTypeValue.key === category.key
                                : null
                            }
                          />
                          <label htmlFor={category.key} className="ml-2 px-2">
                            {category.name}
                          </label>
                        </div>
                      );
                ...
                <div className="addbutton mx-2">
                  <Button
                    ...
                    className="btn-sm px-2 py-2 bg-gradients border-0"
                    type="button"
                    label="Add Itinerary"
                  />
                </div>
                    ...
                    <div className="modal-content">
                      <div className="savebn text-right d-flex justify-content-between align-items-center mt-2 d-flex gap-2">
                        <span className="text-left justify-content-start">
                          <label
                            className="toggle-label h4"
                            htmlFor="flightToggle"
                          >
                            Add Itinerary
                          </label>
                        </span>
                        <span
                          className="close text-right justify-content-end "
                          onClick={handleCloseItineraryClick}
                        >
                          &times;
                        </span>
                      </div>
                      ...
                      <div className="itinerary-form">
                        <div className="form-row d-flex align-content-stretch gap-2">
                          <div className="calendar-item">
                            <FloatLabel>
                              <InputText
                                className="journeyField"
                                id="onwardJourneyFrom"
                                value={newItinerary.onwardJourneyFrom}
                                onChange={(e) =>
                                  handleInputChange("onwardJourneyFrom", e)
                                }
                              />
                              <label htmlFor="onwardJourneyFrom">
                                Onward From
                                <span className="text-danger px-1">*</span>
                              </label>
                            </FloatLabel>
                          ...
                          <div className="calendar-item">
                            <FloatLabel>
                              <InputText
                                className="journeyField"
                                id="onwardJourneyTo"
                                value={newItinerary.onwardJourneyTo}
                                onChange={(e) =>
                                  handleInputChange("onwardJourneyTo", e)
                                }
                              />
                              <label htmlFor="onwardJourneyTo">
                                Onward To
                                <span className="text-danger px-1">*</span>
                              </label>
                            </FloatLabel>
                          ...
                          <div className="calendar-item col-width">
                            <FloatLabel>
                              <Calendar
                                id="onwardDepartureDate"
                                minDate={formData.travelDepartureDate}
                                maxDate={formData.travelArrivalDate}
                                dateFormat="dd-M-yy"
                                value={newItinerary.onwardDepartureDate}
                                onChange={(e) =>
                                  handleInputChange("onwardDepartureDate", e)
                                }
                                showIcon
                              />
                              <label htmlFor="onwardDepartureDate">
                                Departure Date
                                <span className="text-danger px-1">*</span>
                              </label>
                            </FloatLabel>
                          ...
                          <div className="calendar-item">
                            <FloatLabel>
                              ...
                              <InputText
                                id="onwardTime"
                                className="onwardPreferredTime"
                                maxLength={250}
                                value={newItinerary.onwardTime}
                                onChange={(e) =>
                                  handleInputChange("onwardTime", e)
                                }
                              />
                              <label htmlFor="onwardTime">
                                Onward Time
                                <span className="text-danger px-1">*</span>
                              </label>
                            </FloatLabel>
                          ...
                          <div className="calendar-item">
                            <FloatLabel>
                              <InputText
                                id="onwardTransportNumber"
                                style={{ width: "170px" }}
                                value={newItinerary.onwardTransportNumber}
                                onChange={(e) =>
                                  handleInputChange("onwardTransportNumber", e)
                                }
                              />
                              <label htmlFor="onwardTransportNumber">
                                Onward Flight/Train No
                                <span className="text-danger px-1">*</span>
                              </label>
                            </FloatLabel>
                          </div>
                          <div className="calendar-item">
                            <FloatLabel>
                              <InputNumber
                                ...
                                maxLength={250}
                                invalid={newItinerary.budget === 0}
                                value={newItinerary.budget}
                                onChange={(e) => handleInputChange("budget", e)}
                              />
                              <label htmlFor="onwardBudget">
                                Fare<span className="text-danger px-1">*</span>
                              </label>
                            </FloatLabel>
                          </div>
                          <div className="calendar-item flex-grow-1">
                            <FloatLabel className="w-100 car-note">
                              <InputText
                                id="onwardJourneyNote"
                                className="w-100"
                                value={newItinerary.onwardJourneyNote}
                                onChange={(e) =>
                                  handleInputChange("onwardJourneyNote", e)
                                }
                              />
                              <label htmlFor="onwardJourneyNote">
                                Onward Journey Note
                              </label>
                            </FloatLabel>
                          </div>
                        ...
                        <div className="form-row">
                          <div className="form-group SRT d-flex">
                            <label htmlFor="showReturnFields mr-2">
                              Show Return Trip
                            </label>
                            <InputSwitch
                              ...
                              onChange={(e) => {
                                ...
                                // }
                                if (
                                  newItinerary.onwardJourneyFrom !== null &&
                                  newItinerary.onwardJourneyTo !== null
                                ) {
                                  setNewItinerary({
                                    ...newItinerary,
                                    returnJourneyFrom:
                            ...
                            <div className="form-row2 gap-2">
                              <div className="calendar-item">
                                <FloatLabel>
                                  <InputText
                                    className="journeyField"
                                    id="returnJourneyFrom"
                                    value={newItinerary.returnJourneyFrom}
                                    onChange={(e) =>
                                      handleInputChange("returnJourneyFrom", e)
                                    }
                                  />
                                  <label htmlFor="returnJourneyFrom">
                                    Return From
                                    <span className="text-danger px-1">*</span>
                                  </label>
                                </FloatLabel>
                              ...
                              <div className="calendar-item">
                                <FloatLabel>
                                  <InputText
                                    className="journeyField"
                                    id="returnJourneyTo"
                                    value={newItinerary.returnJourneyTo}
                                    onChange={(e) =>
                                      handleInputChange("returnJourneyTo", e)
                                    }
                                  />
                                  <label htmlFor="returnJourneyTo">
                                    Return To
                                    <span className="text-danger px-1">*</span>
                                  </label>
                                </FloatLabel>
                              ...
                              <div className="calendar-item col-width">
                                <FloatLabel>
                                  <Calendar
                                    ...
                                    minDate={
                                      newItinerary.onwardDepartureDate
                                        ? newItinerary.onwardDepartureDate
                                        : formData.travelDepartureDate
                                    }
                                    maxDate={formData.travelArrivalDate}
                                    dateFormat="dd-M-yy"
                                    value={newItinerary.returnArrivalDate}
                                    onChange={(e) =>
                                      handleInputChange("returnArrivalDate", e)
                                    }
                                    showIcon
                                  />
                                  <label htmlFor="returnArrivalDate">
                                    Arrival Date
                                    <span className="text-danger px-1">*</span>
                                  </label>
                                </FloatLabel>
                              ...
                              <div className="calendar-item">
                                <FloatLabel>
                                  ...
                                  <InputText
                                    ...
                                    className="onwardPreferredTime"
                                    maxLength={250}
                                    value={newItinerary.returnTime}
                                    onChange={(e) =>
                                      handleInputChange("returnTime", e)
                                    }
                                  />
                              ...
                              <div className="calendar-item">
                                <FloatLabel>
                                  <InputText
                                    id="returnTransportNumber"
                                    style={{ width: "170px" }}
                                    value={newItinerary.returnTransportNumber}
                                    onChange={(e) =>
                                      handleInputChange(
                                        "returnTransportNumber",
                                        e
                                  ...
                                  />
                                  <label htmlFor="returnTransportNumber">
                    ...
                    <div>
                      ...
                      >
                        ...
                        <Column
                          ...
                          body={(rowData) =>
                            formatDate(rowData.onwardDepartureDate)
                          }
                          headerClassName="custom-header"
                        />
                        <Column
                        ...
                        <Column
                          ...
                          header="Arrival Date"
                          body={(rowData) =>
                            formatDate(rowData.returnArrivalDate)
                          }
                          headerClassName="custom-header"
                        />
                        <Column
                          ...
                          body={(rowData, { rowIndex }) => (
                            ...
                            >
                              ...
                              <Button
                                ...
                                icon={<DeleteIcon />}
                                type="button"
                                onClick={() => openConfirmationDialog(rowIndex)}
                                style={{
                                  backgroundColor: "white",
                                  color: "black",
                                  border: "none",
                ...
                <p className="mx-2">
                  Note: Kindly attach the 3 quotes/routes provided by Travel
                  Agent for comparison. If the least cost-saving route is not
                  taken, kindly provide the reason below.
                </p>
                  ...
                  {attachments.length > 0 && (
                    ...
                    >
                      <Column
                        ...
                        header="Title"
                        headerClassName="custom-header"
                        body={(rowData) => OnwardJourneyLink(rowData)}
                      />
                      <Column
                        header="Actions"
                        headerClassName="custom-header"
          ...
          >
            <Button
              type="button"
              className="back-button-draft mb-3"
